...
class CsvImportCommandTest extends KernelTestCase
{
    public function testExecute()
    {
        ...
        $commandTester4 = new CommandTester($command);
        ...
        $commandTester3->execute(['path' => 'stock3.txt', 'test' => 'test']);
        $commandTester4->execute(['path' => 'stock4', 'test' => 'test']);

        // the output of the command in the console
        $output0 = $commandTester0->getDisplay();
        $output1 = $commandTester1->getDisplay();
        $output2 = $commandTester2->getDisplay();
        $output3 = $commandTester3->getDisplay();
        $output4 = $commandTester4->getDisplay();

        $this->assertContains('Successful!', $output0);
        $this->assertContains('Invalid file!', $output1);
        $this->assertContains('Successful!', $output2);
        $this->assertContains('Successful!', $output3);
        $this->assertContains('File not found!', $output4);
    }
}